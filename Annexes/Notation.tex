\vspace*{0.3cm}
	...
	Si~$G$ est un graphe fini connexe, on notera~$L_G$\marginnote{$L_G$} le laplacien de~$G$. Dès lors, si on note~$\Div^0(G)$\marginnote{$\Div^0(G)$} l'ensemble des diviseurs de~$G$ de degré nulle, on a clairement l'inclusion~$\Princ(G) \subset \Div^0(G)$. On définit alors la \emph{Jacobienne} de~$G$ comme étant le groupe~$\Jac(G)$ donné par :
	\[
